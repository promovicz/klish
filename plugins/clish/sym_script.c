...
CLISH_PLUGIN_OSYM(clish_script)
{
	...
	if(pager_command && clish_shell__get_interactive(this)) {
	  use_pager = clish_shell__get_pager(this);
	  if(use_pager)
		use_pager = clish_command__get_pager(cmd);
	}
	/* Do not enable pager for interruptible commands */
	if(cmd && clish_command__get_interrupt(cmd)) {

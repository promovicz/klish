...
PyObject *clish_python_init(void)
{
	...
	if(Py_IsInitialized()) {
		//fprintf(stderr, "clish_python_init: Python already initialized\n");
	} else {
		//fprintf(stderr, "clish_python_init: Initializing python\n");
		Py_Initialize();
	}

	/* register module */
	//fprintf(stderr, "clish_python_init: Initializing module\n");
#ifdef PYTHON3
	module = PyState_FindModule(&module_definition);
	if(module) {
		//fprintf(stderr, "clish_python_init: Module already present\n");
		goto done;
	} else {
		//fprintf(stderr, "clish_python_init: Creating module instance\n");
		module = PyModule_Create(&module_definition);
		if(!module) {
			goto fail;
		}
		//fprintf(stderr, "clish_python_init: Adding module instance\n");
	...
	if(!module) {
		//fprintf(stderr, "clish_python_init: Failed to initialize module\n");
		goto fail;
	}
#endif

	/* register classes */
	//fprintf(stderr, "clish_python_init: Registering classes\n");
...
#endif
	//fprintf(stderr, "clish_python_init: DONE\n");
	return module;

fail:
	//fprintf(stderr, "clish_python_init: FAILED\n");
	return NULL;
}
